...
require "log4r-color/formatter/formatter"
require "log4r-color/GDC"
require "log4r-color/MDC"
require "log4r-color/NDC"
  
module Log4r
  # See log4r-color/formatter/patternformatter.rb
  class PatternFormatter < BasicFormatter
    ...
    def initialize(hash={})
      ...
      @date_pattern = self.class.iSO8601 if @date_pattern.nil? and @date_method.nil?
    ...
    def self.create_format_methods(pf) #:nodoc:
      ...
      while true # work on each match in turn
        ...
        ebuff << match[3] + "s"
      	...
      	if ( match[5] == 'X' && match[6] != nil ) then
      	  ...
      	  args << directiveTable[match[5]].gsub("DTR_REPLACE", match[6]).gsub(match6sub,'')
        else
